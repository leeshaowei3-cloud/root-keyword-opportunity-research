...
For every strict title-supply summary, report method and tokenizer versions, exact query syntax, count method, frozen `hl`/`gl`/desktop/`google_web`/`pws=0`/`filter=0`/`nfpr=1`/checked-date context and its key, exact count, `lower_bound_gt_0_30` count, CAPTCHA/block/loop stops, query-integrity holds, method/context mismatches, and missing observations. Different context keys stay in separate cohorts and cannot share a count. A single displayed `allintitle:` estimate is legacy-only; report the batch-specific classic denominator and strict-method reuse count in the migration ledger. Never label `StrictTitleSupplyRatio` as KGR.
